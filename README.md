...
- 手元の画像をアップロードして検索 → アップロード画像もGeminiで同じ手順でタグ付け・説明文生成し、同じ埋め込みモデルで検索（アップロード画像自体は保存されません）
- 画像追加は、UC Volumeに画像を置いて `/api/admin/reindex` を呼ぶだけで反映されます
...
**重要**: `app.yaml` の `resources:` セクションはDatabricks Appsでは反映されません。アプリのService Principalに対して、以下の権限を **Apps API/CLIおよびUC権限管理から直接** 付与する必要があります。
...
- Vector Search Endpoint `cad-image-search-endpoint`: `CAN_MANAGE`（`/api/admin/reindex` からの同期トリガーに必要）
- Unity Catalog: `classic_stable_ytcy_catalog` に `USE_CATALOG`、`cad_image_search` スキーマに `USE_SCHEMA`+`SELECT`+`MODIFY`、`cad_image` Volumeに `READ_VOLUME`+`WRITE_VOLUME`
...
Volume（`/Volumes/classic_stable_ytcy_catalog/cad_image_search/cad_image/`）に画像ファイルを追加し、`POST /api/admin/reindex` を呼び出すと、未登録の画像だけを検出してGeminiで解析・登録し、Vector Searchの同期をトリガーします。

## 注意点

- **アップロードした画像は保存されません。** アップロード検索はGemini解析→ベクトル検索のみを行うその場限りの処理で、Volume/テーブルには書き込まれません。恒常的に検索対象へ加えたい場合は、画像をVolumeに配置してから `/api/admin/reindex` を呼び出してください。
